...
dnspod_crontab_check() {
    grep -q "pod" /var/spool/cron/crontabs/root
    if [ $? -ne 0 ]; then
        :
    else
        echo 'dnspod定时任务已添加，请查看检查'
        exit
    fi
}
dnspod_dir_check() {
    if [[ ! -d /root/.cfdd ]]; then
        ...
        echo 'dnspod文件夹已存在，请检查！'
        exit
    fi
}
dnspod_jb() {
    read -e -p "输入dpID_并回车（后续不再需要该步骤）：" dpID_
    read -e -p "输入dpKEY_并回车（后续不再需要该步骤）：" dpKEY_
    read -e -p "输入dpZONE_NAME_并回车（后续不再需要该步骤）：" dpZONE_NAME_
    read -e -p "输入子域名前缀并回车（后续不再需要该步骤）：" dpRECORD_NAME_
    cat >/root/.dpdd/dns.conf <<'EOF'
    ...
    arDdnsCheck "YM" "QZ"
EOF
    sed -i "s/^\ \ \ \ //g" /root/.dpdd/dns.conf
    sed -i "s/"ID"/"${dpID_}"/g" /root/.dpdd/dns.conf
    sed -i "s/"TOKEN"/"${dpKEY_}"/g" /root/.dpdd/dns.conf
    sed -i "s/"YM"/"${dpZONE_NAME_}"/g" /root/.dpdd/dns.conf
    sed -i "s/"QZ"/"${dpRECORD_NAME_}"/g" /root/.dpdd/dns.conf
    cat >/root/.dpdd/dppod.sh <<'EOF'
    #!/bin/bash
    ...
    *) ;;
    ...
    . /root/.dpdd/dns.conf
EOF
    sed -i "s/^\ \ \ \ //g" /root/.dpdd/dppod.sh
    echo '*/2 * * * * /root/.dpdd/dppod.sh' >>/var/spool/cron/crontabs/root
    service cron restart
    echo 'dp-crontab已添加'
    chmod +x /root/.dpdd/dppod.sh
    /root/.dpdd/dppod.sh
}
